(* Code generation for Makefile DSL to OCaml *)
...
let ocaml_of_var_def (name : string) (var_def : var_def) : string =
  let rec gen_expr var_def =
    ...
    | VarLiteral words -> 
        ...
        Printf.sprintf "List.flatten [[%s]]" (String.concat "; " quoted)
    | VarRef var_name ->
  ...
  in
  Printf.sprintf "let rec var_%s _ = %s" name (gen_expr var_def)
...
let generate_ocaml_code (env : env) (rules : rule list) : string =
  ...
  in
  let _ = List.map ocaml_of_rule rules in (* Silence unused var warning *)
